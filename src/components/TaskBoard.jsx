    ...
    const onDragEnd = (result) => {
        ...
        if (type === 'CARD') {
            ...
            if (sListMeta.id === dListMeta.id) {
                ...
                if (sListMeta.__isPinned === dListMeta.__isPinned) {
                    tempDestListArray = tempSourceListArray;
                }
                const dListIndex = tempDestListArray.findIndex(l => l.id === dListMeta.id);

                if (sListIndex === -1 || dListIndex === -1 ||
                    !Array.isArray(tempSourceListArray[sListIndex].cards) ||
                    !Array.isArray(tempDestListArray[dListIndex].cards)) return;
        ...
        <DragDropContext onDragEnd={onDragEnd}>
            <PinnedListsArea
                ...
                listProps={{
                    ...
                    isPinnedContext: true // ++ Adiciona a prop aqui para PinnedListsArea ++
                }}
                    ...
                    >
                        {(Array.isArray(displayedRegularLists) ? displayedRegularLists : []).map((list, index) => (
                            <ListColumn
                                ...
                                onDeleteCard={handleDeleteCard}
                                onUpdateTaskStatus={handleUpdateTaskStatus}
                                isPinnedContext={false} // ++ Adiciona a prop aqui para listas regulares ++
                            />
                        ))}
